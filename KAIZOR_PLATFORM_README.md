...
```mermaid
flowchart TB
  A[Real-world inputs<br/>documents • tickets • chats • screenshots • signals • requests] --> B[Understanding]
  B --> C[Reasoning and decision support]
  C --> D[Governed execution]
  D --> E[Outcomes and reporting]
  E --> F[Memory and reusable knowledge]
  F --> B
...
sequenceDiagram
  participant User
  participant Kaizor
  participant Workflow as Workflow/System

  User->>Kaizor: task, issue, question, or goal
  Kaizor->>Kaizor: understand context and reduce noise
  Kaizor->>User: propose meaning, options, and next steps
  User->>Kaizor: confirm, refine, or redirect
  Kaizor->>Workflow: support safe execution
  Workflow-->>Kaizor: result, status, or evidence
  Kaizor->>User: summary, decision support, and next action
```
...
flowchart LR
  ...
  K --> O1[Understanding]
  K --> O2[Guidance]
  K --> O3[Workflow support]
  K --> O4[Governed action]
```
...
```mermaid
flowchart TD
  A[User intent or workflow trigger] --> B{Action impact level}
  B -->|Low-risk| C[Assist or automate]
  B -->|Higher-impact| D[Require confirmation]
  C --> E[Outcome]
  D --> E
  E --> F[Reviewable history and learning]
```
...
```mermaid
flowchart LR
  A[Inputs and interactions] --> B[Understanding]
  B --> C[Useful outputs]
...
flowchart LR
  ...
  P --> C4[Operations and reporting tracks]
